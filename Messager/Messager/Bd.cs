...
namespace Messager
{
    public class Bd
    {
        ...
        public static List<User> getUserMess1(string name) => (from user in new Mess1().User where user.name.Contains(name) select user).ToList();
        public static List<User> getUserMess2(string name) => (from user in new Mess2().User where user.name.Contains(name) select user).ToList();
